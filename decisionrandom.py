...
# Fit the Random Forest classifier with multiple parameters to simplify the trees
rf_model = RandomForestClassifier(
    random_state=42, 
    max_depth=3,               # Limit depth of the tree
    min_samples_split=10,      # Increase minimum samples required to split a node
    min_samples_leaf=5,        # Increase minimum samples required to be at a leaf node
    max_leaf_nodes=10,         # Limit maximum number of leaf nodes
...
plt.title('Tree for Distinguishing Benign vs. Attack Traffic')
plt.show()
