...
# Filter the dataframe to include only 'DDoS' attacks
df = df[df['Label'] == 'DDoS']
...
# Encode the target labels to numeric values (binary: 1 for DDoS)
...
def plot_decision_boundary(clf, X, y, title, feature_names):
    ...
    plt.contourf(xx, yy, Z, alpha=0.4, cmap=ListedColormap(('red')))  # Only 'DDoS' in red
    ...
    # Create legend
    handles = [plt.Line2D([0], [0], marker='o', color='w', markerfacecolor='red', markersize=10, label='DDoS')]
    plt.legend(handles=handles, loc='best')
    
    plt.show()

# Plot decision boundary using PCA-reduced features (only DDoS)
plot_decision_boundary(xgb_model, X_test, y_test, "Decision Boundary for XGBoost Model (DDoS Only, PCA-reduced)", ['PC1', 'PC2'])
