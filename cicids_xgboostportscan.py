...
# Filter the dataframe to only contain 'PortScan'
df = df[df['Label'] == 'PortScan']
...
# Since we're focusing only on 'PortScan', we don't need to split into train/test sets here
# Instead, we'll use the entire dataset for training and plotting
X_train, X_test, y_train, y_test = X_pca, X_pca, y_encoded, y_encoded
...
if xgb_model.get_booster().get_dump():
    ...
    def plot_decision_boundary(clf, X, y, title, feature_names):
        ...
        plt.contourf(xx, yy, Z, alpha=0.4, cmap=ListedColormap(('red')))  # Only 'PortScan'
        scatter = plt.scatter(X[:, 0], X[:, 1], c=y, s=20, edgecolor='k', cmap=ListedColormap(('red')))
        plt.title(title)
        plt.xlabel(feature_names[0] + ' (PCA)')
        plt.ylabel(feature_names[1] + ' (PCA)')
        
        # Create legend
        handles = [plt.Line2D([0], [0], marker='o', color='w', markerfacecolor='red', markersize=10, label='PortScan')]
        plt.legend(handles=handles, loc='best')
        
        plt.show()

    # Plot decision boundary using PCA-reduced features
    plot_decision_boundary(xgb_model, X_test, y_test, "Decision Boundary for XGBoost Model (PortScan)", ['PC1', 'PC2'])
